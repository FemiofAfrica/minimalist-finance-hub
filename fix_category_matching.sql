-- Improved create_transaction function with robust category handling
...
-- Let's also check if we need the Groceries category
INSERT INTO categories (name, user_id, type)
SELECT 'Groceries', auth.uid(), 'expense'
FROM auth.users
WHERE NOT EXISTS (
  SELECT 1 FROM categories 
  WHERE LOWER(name) = 'groceries' AND type = 'expense' AND user_id = auth.uid()
); 